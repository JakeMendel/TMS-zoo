...
class OthelloDataset:
    def __init__(self, data_root, ood_perc=0., wthor=False, ood_num=1000, generate_dataset=False, max_samples=-1):
        ...
        criteria = lambda fn: fn.suffix == "pgn" if wthor else str(fn.name).startswith("liveothello")

        if generate_dataset:
            print(f"Generating synthetic dataset with {ood_num} games")
            ...
            if ood_num > ood_num:
                with open(data_root / f'synthetic/gen10e5_{t_start}.pickle', 'wb') as handle:
                    pickle.dump(self.sequences, handle, protocol=pickle.HIGHEST_PROTOCOL)

        elif 'synthetic' in data_root.absolute().as_posix():
            #! User wants to load a synthetic dataset
            ...
            for f in bar:
                ...
                if max_samples != -1 and len(self.sequences) >= max_samples:
                    break 
                if not str(f.name).endswith(".pickle"):
                    continue
        ...
        elif 'championship' in data_root.absolute().as_posix():
            print("Loading championship games")
            for fn in data_root.iterdir():
                if criteria(fn):
                    with open(data_root/fn, "r") as f:
                        pgn_text = f.read()
            ...
            print("No data found, please generate synthetic data first by setting generate_dataset=True")
